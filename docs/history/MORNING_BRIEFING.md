...
> **Historical document.** This build log describes the repo as it was *before*
> the modern-framework refactor, so its file paths (`src/hexpionage.game.php`,
> `src/hexpionage.js`) no longer exist. Kept as a record of the Phase 3 fix wave.
> For current layout see [`ONBOARDING.md`](../../ONBOARDING.md).
