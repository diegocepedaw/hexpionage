...
> - `docs/specs/STATE_MODEL.md` (A4 output).
> - `docs/specs/STATE_MACHINE.md` (A5 output).
> - `docs/history/PLAN.md` §Deliverable 3 / §Deliverable 4.
>
> **Hard rules followed**:
...
docs/history/PLAN.md §Deliverable 4 lists Phase 0–5. Current artifacts:
- `docs/specs/BGA_PRIMER.md`, `docs/specs/BGA_CHECKLIST.md`, `docs/specs/BGA_PATTERNS.md` — A1 outputs (Phase 0). **EXISTS**.
- `rulebook.md` — rules formalization (was supposed to be `docs/specs/RULES.md`). **EXISTS but RENAMED**.
- `DECISIONS.md` — owner decisions. **EXISTS**.
- `design/MANIFEST.md` (A3 output) — referenced by PLAN but not in `/Users/dcepeda/Documents/hexpionage/` per directory listing. Only `design/` directory is listed, no MANIFEST file visible. **MISSING-33**: A3 deliverable not visible.
- `docs/specs/STATE_MODEL.md` (A4 output) — Phase 1. **EXISTS**.
- `docs/specs/STATE_MACHINE.md` (A5 output) — Phase 1. **EXISTS**.
- `docs/specs/UI_SPEC.md` (A6 output) — Phase 1. **NOT VISIBLE**.
- `docs/testing/SCENARIOS.md` (A10 output) — Phase 1. **NOT VISIBLE**.
- `docs/specs/CONTRACT.md` (A11 output) — Phase 1. **NOT VISIBLE**.
- `docs/specs/QA_SPEC_REVIEW.md` (this doc, A9 output) — Phase 2. **IN PROGRESS**.
...
**[FINDING-41] (S3)** — `rulebook.md` is the artifact, but docs/history/PLAN.md repeatedly refers to `docs/specs/RULES.md`. Naming drift; harmless but confusing.
...
**[FINDING-44] (S3)** — PLAN A5 description: "Every legal action in `docs/specs/RULES.md` maps to exactly one `(state, action)` pair." STATE_MACHINE §12.1 confirms 16 actions, all mapped. **OK.**
...
| F-41 | S3 | rulebook.md vs docs/specs/RULES.md naming |
