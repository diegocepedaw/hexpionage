...
These are the printed rulebook pages and box-back art. The 3 rulebook PNG pages (`rules_templated_nice_01/02/03.png`) **may** be used as in-game help-modal images if the owner wants — but the canonical machine-readable rules live in `specs/RULES.md` (A2 deliverable), not these images. Marking `unused / print-only` for the BGA play surface; the frontend agent (A8) can revisit if a help-modal slideshow is desired.
