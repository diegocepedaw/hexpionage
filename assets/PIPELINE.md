...
**Cross-references**: `assets/MANIFEST.md` (input inventory), `assets/MISSING.md` (asset gaps that must be authored before running the pipeline), `rulebook.md` §2 (component naming).
...
The script lives at the repo root (or under `assets/`) and assumes:
- `$SRC` = `/Users/dcepeda/Downloads/final_printing` (read-only)
- `$DST` = `<repo>/src/img`
- `$MASK` = a 450×450 hex alpha mask PNG, **already authored once** and committed at `assets/hex_mask_450.png` (one-time setup; the geometry is taken from `example_finished_tile_with_shape.png`).
...
# These SVGs are committed under assets/generated/*.svg by a one-time authoring task.
# Build step rasterizes them to PNG. If BGA loads SVG directly, this section is skipped.
for svg in assets/generated/*.svg; do
  [[ -e "$svg" ]] || continue
...
After `build_assets.sh` completes, a separate `validate_assets.sh` checks that every "BGA sprite" entry in `assets/MANIFEST.md` is represented by a cell in the produced sprite sheets.
...
These artifacts must exist before the build script can run. They are committed to the repo under `assets/`:

1. **`assets/hex_mask_450.png`** — a 450×450 alpha mask whose opaque region is the hex-shape footprint. Source the geometry from `example_finished_tile_with_shape.png`. Author once.
2. **`assets/generated/*.svg`** — generated SVG files for score marker, dice faces (see `MISSING.md`). Author once or per design iteration.
