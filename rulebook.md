...
> **TODO(B-01)**: The exact hex layout (row count, columns per row, total Field hex count, position of the score track and player roster) must be derived from `game_board_print.png`. Implementers must publish the canonical coordinate map in `assets/BOARD_LAYOUT.md`. Without this, all references to specific hexes in this document use abstract names (`top-left`, `bottom-row[i]`, etc.).
...
> **Why a two-step flow [D-26]**: a single-step "blind pre-commit" (the player choosing keep/return BEFORE seeing the drawn tile) would deprive the player of an informed decision — see F-13 / D-26-CANDIDATE in `specs/QA_SPEC_REVIEW.md`. Splitting into a sub-state with a private reveal preserves both the strategic choice and the bag-composition privacy on `return` (per [D-20]).
...
- ✅ Owner has resolved blocking ambiguities via D-14 through D-19 (consolidated in [DECISIONS.md](DECISIONS.md)):
