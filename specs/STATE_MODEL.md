...
>
> **Scope**: server-side state only. State-machine transitions are A5's job (see `specs/STATE_MACHINE.md`); UI is A6's job (see `specs/UI_SPEC.md`).
>
...
For each rule-class in `rulebook.md` §6, this section gives one or two queries proving the state model can answer the legality/effect question. This is the validation contract per PLAN A4 description: "every rule in `specs/RULES.md` can be expressed as a query against this schema."
